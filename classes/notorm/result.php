...
class NotORM_Result extends NotORM_Abstract implements \Iterator, \ArrayAccess, \Countable {
     ...
     */
    function __construct($table, NotORM_Db $notORM, $single = false) {
        $this->table = $table;
        $this->notORM = $notORM;
        $this->single = $single;
    ...
    protected function query($query, $parameters) {
        ...
        if (!empty($this->notORM->_config["profiling"])) {
            $benchmark = \Profiler::start("Database ({$this->notORM->_config['instance']})", $query);
        }
        try {
